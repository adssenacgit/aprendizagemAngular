  ...
  ngOnInit(): void {
    ...
    this.ofertasService.ObterOfertasPeloId(this.ofertaId).subscribe(resultado => {
      ...
      this.formulario = new FormGroup({
        ofertaId: new FormControl(resultado.id),
        cursoId: new FormControl(resultado.cursoId, [Validators.required]),
        descricao: new FormControl(resultado.descricao, [Validators.required, Validators.minLength(1), Validators.maxLength(50)]),
        codigo: new FormControl(resultado.codigo, [Validators.required, Validators.minLength(1), Validators.maxLength(50)]),
        inicioOperacao: new FormControl(dataInicioOperacao, [Validators.required]),
        status: new FormControl(resultado.status.toString(), [Validators.required])
      })
    })
  }
